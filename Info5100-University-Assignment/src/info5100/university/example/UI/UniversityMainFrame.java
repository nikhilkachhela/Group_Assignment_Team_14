...
import info5100.university.example.UI.Registrar.RegistrarWorkAreaJPanel;
...
// Add this helper method to find the student profile
private StudentProfile findStudentProfile(User user) {
    // First try to get from user's profile reference
    if (user.getProfileReference() instanceof StudentProfile) {
        return (StudentProfile) user.getProfileReference();
    }
    
    // Otherwise search in student directory
    ArrayList<StudentProfile> students = department.getStudentDirectory().getStudentList();
    
    // Match by person ID if available
    Person userPerson = user.getPerson();
    if (userPerson != null) {
        for (StudentProfile student : students) {
            if (student.isMatch(userPerson.getPersonId())) {
                return student;
            }
        }
    }
    
    // If still not found, try to match by index (for demo purposes)
    // This assumes students are created in order
    String username = user.getUsername();
    if (username.startsWith("student")) {
        try {
            // Extract number from username like "student1", "student2", etc.
            int index = Integer.parseInt(username.substring(7)) - 1;
    ...
    // Default to first student if available (for testing)
    if (!students.isEmpty()) {
